...
%        MainPath_RunReverse = '';
         ...
         xlabel(ha(end),'Time (s)');
         set(ha(end), 'XTickLabelMode', 'auto')
         set(ha(1:end), 'TickLength',[0.005 0.0125]); 
         
          %Axes properties
         set(ha(1:end), 'FontSize', 14);
         set(ha(1:end), 'FontName', 'Helvetica');
         set(ha(1:end), 'box', 'on');
         set(ha(1:end),'Linewidth',1.2);
         
         % same time zoom for all plots
         linkaxes(ha, 'x');
          
         set(hTime, 'color', 'w')
         set(hTime, 'PaperUnits', 'centimeters');
