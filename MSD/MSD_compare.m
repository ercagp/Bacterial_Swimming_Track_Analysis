...
bug_i = [38,44,114,116,289,374,514,524,525,599,638,833,853,883]; 
non_motile_bugs = [];
bug_i = bug_i(~ismember(bug_i,non_motile_bugs));
...
tau_i = 1:length(r)-1;
    ...
    for j = tau_i
    MSD_bug{i_loop}(j) = MSD(r,j,fps);
    end

tau = tau_i./fps;

p{i_loop} = plot(tau,MSD_bug{i_loop},'.','MarkerSize',13,...
    'Color',c_map{i_loop});
