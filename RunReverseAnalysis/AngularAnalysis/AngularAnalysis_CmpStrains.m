...
%         printfig(hfDensityPlt,fullfile(ExpPath,['[20201121]' StrainLabels{j} '_[IPTG]_' num2str(IPTG_AngBA) 'uM_AngleBeforeRunAfterRun.pdf']),'-dpdf')
    end
    ...
    printfig(hf,fullfile(ExpPath,[Stamp StrainLabels{j} '_NormalizedTurnAngles.pdf']),'-dpdf')
    printfig(hf,fullfile(ExpPath,[Stamp StrainLabels{j} '_NormalizedTurnAngles.png']),'-dpng')
     
    hf_CmpStr = figure(100); 
    %Find the dataset for selected [IPTG] value 
    kSelect = find([TurnA{:,1}] == IPTGSelect); 
    TurnAngleSelect = TurnA{kSelect,2};
    TotalTrajDurSelect = TotalTrajDur{kSelect,2};
    HistPara.Label.Title = IPTG_TurnAngle{kSelect}; 
    TAHist_CmpStr{j} = AngleDist(TurnAngleSelect,TotalTrajDurSelect,HistPara,hf_CmpStr); 
    hold on     
end
legend([TAHist_CmpStr{1:end}],StrainLabels{1:end},'Location','NorthWest')
printfig(hf_CmpStr,fullfile(ExpPath,[Stamp 'StrainCmp_[IPTG]_' num2str(TurnA{kSelect,1}) 'uM_TurnAngles.pdf']),'-dpdf')
printfig(hf_CmpStr,fullfile(ExpPath,[Stamp 'StrainCmp_[IPTG]_' num2str(TurnA{kSelect,1}) 'uM_TurnAngles.png']),'-dpng')
