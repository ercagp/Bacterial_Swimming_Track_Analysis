...
ExpPath = '/Users/ercagpince/Dropbox/Research/NikauBackup/Data/RunReverseAnalysis/RunAnalysis/SpeedAnalysis/Distributions/MeanRunSpeed/Cmp_MeanRunSpeeds';
...
for j = 1:length(StrainLabels) 
    ...
    for i = 1:length(Files) 
        ...
        RunCell{i} = RunBreakDownv3(Speeds,T,fps);  %RunBreakDownv3(S,T,fps) --> The order
        ...
        InstRunSpeed{i} = cell2mat(RunCell_Flt{i}(:,2)); 
        Temp = cell2mat(RunCell_Flt{i}(:,1)); 
        %Run durations
        RunDur{i} = Temp(:,1); 
        %Mean speed of runs 
        MeanRunSpeed{i} = Temp(:,2);  
        %Median speed of runs 
        %MedRunSpeed{i} = Temp(:,3);
    ...
    for k = 1:size(InstRS,1)
             ...
             printfig(hf,fullfile(ExpPath,['[20201129][MeanRunSpeedCMP]Counts_' StrainLabels{j}...
