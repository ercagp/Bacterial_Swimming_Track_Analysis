...
ColorMap_Mean = linspecer(2,'Sequential'); 
    ...
    for i = 1:length(Files) 
        ...
        switch observable 
               ...
               case 'meanV'
                     ...
                     PltPara.NormSwitch = ''; 
                     PltPara.FaceColor = ColorMap_Mean(2,:); 
                     PltPara.EdgeColor = ColorMap_Mean(1,:) ; 
                     PltPara.Label.Title  = {[StrainLabels{j} ' @ ' num2str(IPTG(i)) '\muM [IPTG] - ' VideoDates{i}], ROI{i}};
                     PltPara.Label.XAx = '<V_{Run}> (\mum/sec)';
                     ...
                     X.X = MeanV;
                     X.RunDur = RunDur; 
                     DistH = RunDist(X,PltPara,hFig);
                     legend([DistH{1:end}],{'Weighted','Native'},'FontSize',10);
...
%         The input has now two components  
          ...
          %Weighted values
          RunDistH{1} = histogram('BinEdges',Edges,'BinCounts',NW,'Normalization',Norm,'LineWidth',2.5,...
                         'DisplayStyle','stairs','EdgeColor',EdgeColor);
          hold on 
          %Native Values
          RunDistH{2} = histogram(XValues,Edge,'Normalization',Norm,...
