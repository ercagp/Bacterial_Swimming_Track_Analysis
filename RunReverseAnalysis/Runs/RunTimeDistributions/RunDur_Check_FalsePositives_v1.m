%% Plot Run Duration(aka Times) Distributions at different run speed bins to  
...
for j = 1:length(StrainLabels)
    ...
    for i = 1:length(Files)
     ...
     RDurCell = matchrepeat(IPTG,RunDur);
     %Define colormaps for different & selected [IPTG] values 
     ColorMap_Edges = linspecer(size(RDurCell,1),'qualitative');
     ColorMap_Sel = linspecer(size(StrainLabels,2),'qualitative'); 
     %Preallocate 
     IPTG_S = cell(size(RDurCell,1),1); 
     IPTG_Num = zeros(size(RDurCell,1),1); 
     h_PDF = cell(size(RDurCell,1),1); 
     h_CDF = cell(size(RDurCell,1),1); 
        
     for k = 1:size(RDurCell,1)    
         RD = RDurCell{k,2};
         
         IPTG_Num(k) = RDurCell{k,1}; 
         ...
         switch observable 
                case 'RunDurPDF'
                  ...
                  if IPTG_Num(k) == IPTG_Sel
                     ...
                     hand_Select{j} =  HistRunDur(RD,PPSelect,hfSelect);
                     drawnow(); 
