...
function Hist = HistRunDur(X,PltPara,hFig)
         ...
         %Preallocate vector for run durations of different bins
         RunDurBin = cell(length(SpeedEdges)-1,1);
         SpeedEdgeStr = cell(length(SpeedEdges)-1,1);
         Hist = cell(length(SpeedEdges)-1,1);
         %Perform speed binning 
         [~,~,BinI] = histcounts(X(:,2),SpeedEdges);
         
         
         for i = 1:length(SpeedEdges)-1
             MaskSpeeds = BinI == i;             
             
             %Isolate run durations in the i^th bin 
             RunDurBin{i} = X(MaskSpeeds,1);
             
             %Gamma fit 
             GammaPD = fitdist(RunDurBin{i},'Gamma');
             XFit = linspace(PltPara.Lim.X(1),PltPara.Lim.X(2),1000); 
             
             %Set edge color 
             EdgeColor = ColorMapBins(i,:); 
             
             if strcmp(Norm,'PDF')
                Hist{i} = histogram(RunDurBin{i},DurEdges,...
                            'EdgeColor',EdgeColor,...
                            'Normalization',Norm,...
                            'DisplayStyle','Stairs',...
                            'LineWidth',LineWidth); 
%                 if i == length(SpeedEdges)-1
%                    Y_PDF = pdf(GammaPD,XFit);
%                    plot(XFit,Y_PDF,'-','LineWidth',LineWidth,'Color',EdgeColor);
%                 end
                
             else
                %The part for 1-CDF plot must be updated!!! 
                Hist{i} = histogram('BinEdges',DurEdges,'BinCounts',RunDurBin{i},...
                            'EdgeColor',EdgeColor,...
                            'Normalization',Norm,...
                            'DisplayStyle','Stairs',...
                            'LineWidth',LineWidth); 
             ...
             hold on 
             SpeedEdgeStr{i} = [num2str(SpeedEdges(i)) ' - ' num2str(SpeedEdges(i+1)) ' \mum/s'];
             
         end
         
         legend([Hist{1:end}], SpeedEdgeStr{1:end},'Location','NorthEast');
         hold off
end
