...
ExpPath = 'C:\Users\ercagp\Documents\Rowland_Data\RunReverseAnalysis\EventRates';
mkdir(ExpPath);
...
for j = 1:length(StrainLabels)
    ...
    for i = 1:length(Files) 
        ...
        [TurnFreq_Bin{i,2}, MeanV{i,2}] = TurnFreq_MeanV(Speeds,Results.T,Edge,APara);     
    end
...
%% Functions
 ...
 function [TurnFreq, MeanV_perAcq] = TurnFreq_MeanV(S,T,Edges,AllPara)
...
function pHand = PlotTFV(hf,X,Para)
          ...
          %Mean values
          meanV = mean(cell2mat(X.X),1,'omitnan'); 
          meanTF = mean(cell2mat(X.Y),1,'omitnan');
          std_X = std(cell2mat(X.Y),0,1,'omitnan'); 
          std_Y = std(cell2mat(X.Y),0,1,'omitnan'); 
          errorbar(meanV,meanTF,std_Y,std_Y,std_X,std_X,'.','MarkerSize',32,...
