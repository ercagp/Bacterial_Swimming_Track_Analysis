...
for j = 1:length(CallStrainLabels)
    ...
    %Go through each bug
        ...
        %Split trajectories in segments as odd and even
           ...
           %Check which set has larger mean speed 
           AvgOdd = nanmean(OddSpeeds); 
           AvgEven = nanmean(EvenSpeeds); 
           %Classify slower as backward run and faster as forward 
                if (AvgOdd > AvgEven) || isnan(AvgEven)
                   ForwardCell{k,:} = OddSpeeds;
                   BackwardCell{k,:} = EvenSpeeds;
                else
                   ForwardCell{k,:} = EvenSpeeds;
                   BackwardCell{k,:} = OddSpeeds;    
                end
        end
        
    %Plot Odd&Even Speeds Histogram
    hf_hist = figure; 
    hs_fw = histogram(cell2mat(ForwardCell),bins,'DisplayStyle','Stairs');
    hold on 
    hs_bw = histogram(cell2mat(BackwardCell),bins,'DisplayStyle','Stairs');
    drawnow()
    legend([hs_fw hs_bw],{'FW','BW'});
    ax_hist = gca; 
    ...
    printfig(hf_hist,fullfile(FullExportPath,['FWBWVelDist_' StrainLabel '_' ROI]),'-dpdf')
