...
%Plots for the randomly selected bugs in the 
n_rand = 3; %number of bugs to be plotted 
    for j = 1:n_rand 
    
    select_rand = kind(randperm(length(kind),1)); %Index of the randomly selected bug 
    %Speed values of the selected bug
    S_rand = SpeedStats{i}.allV{select_rand};
    
    
    %Speed distribution of the selected bug
    hf_rand = figure; 
    histogram(S_rand, edgesV,'Normalization','pdf','DisplayStyle','stairs') %Note that the same edgeset is used
    title({['Bug#' num2str(select_rand)],['FOV#' num2str(i)]})
    ErcagGraphics
    printfig(hf_rand,[Folder_analysis filesep 'HistV_Bug_' num2str(select_rand) '_FOV_' num2str(i)],'-dpdf')
    
    %Plot trajectory of the selected bug with speed heatmap 
    traj_selected = figure;
    PlotColouredSpeedTrajectory_ForStruct(S{i},select_rand,maxSpeed,'label'); 
    title({['Bug#' num2str(select_rand)],['FOV#' num2str(i)]})
    savefig(traj_selected,[Folder_analysis filesep 'Traj_Bug_' num2str(select_rand) '_FOV_' num2str(i)])
...
%Save each figure to a .fig file 
