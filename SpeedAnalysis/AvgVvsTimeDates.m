%Compare average speed vs. Time of data taken at different days
%by Ercag Pince
%October 2019 
clearvars;
close all; 
...
FileKeyword = ['avgV_' StrainKeyword]; 
...
for dataI = 1:length(VideoDates)
    ...
    err{dataI} = errorbar(timeVec, avgV, semAvgV,'.');
    hold on 
end
...
printfig(hf_1,fullfile(MainExportPath,'20190924', 'KMT9_AverageSpeedvsTimeDates'),'-dpng')
printfig(hf_1,fullfile(MainExportPath,'20190924', 'KMT9_AverageSpeedvsTimeDates'),'-dpdf')
savefig(hf_1,fullfile(MainExportPath,'20190924', 'KMT9_AverageSpeedvsTimeDates'))
