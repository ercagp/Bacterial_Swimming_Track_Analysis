...
for j = 1:length(StrainLabels) 
    ...
    ColorMap_Edges = linspecer(size(InstV,1),'qualitative');
    ...
    switch observable 
        case 'instV'
             ...
             for k = 1:size(InstV,1)
                ...
                IS.X = InstV{k,2}; %Instantaneous Speeds
                IS.TrajDur = TrajDurV{k,2};
                ...
                InstV_Handle{k} = SpeedDist(IS, PltPara, hf);
                hold on 
             ...
             figure(hf)
             SortandPutLeg(InstV,InstV_Handle,IPTGLabel); 
             drawnow();
             FinalExpPath = fullfile(ExpPath,'InstantaneousSpeed'); 
             if ~exist(FinalExpPath,'dir')
                 mkdir(FinalExpPath)
             end
             printfig(hf,fullfile(FinalExpPath,[Stamp '[InstantaneousSpeed]' StrainLabels{j} '.pdf']),'-dpdf');
             printfig(hf,fullfile(FinalExpPath,[Stamp '[InstantaneousSpeed]' StrainLabels{j} '.png']),'-dpng');
        case 'meanV'
              MeanV_Handle = cell(size(MeanV,1),1);
              for k = 1:size(MeanV,1)
                  IPTGLabel{k} = [num2str(MeanV{k,1}) '\muM'];
        
                  MS.X = MeanV{k,2}; %Mean Speeds
                  ...
                  PltPara.Label.XAx = '<V> (\mum/sec)';
                  MeanV_Handle{k} = SpeedDist(MS, PltPara, hf);
                  hold on 
              ...
              figure(hf)
              SortandPutLeg(MeanV,MeanV_Handle,IPTGLabel); 
              drawnow();
              if ~exist(FinalExpPath,'dir')
                 mkdir(FinalExpPath)
              end
              printfig(hf,fullfile(FinalExpPath,[StampMeanV StrainLabels{j} '.pdf']),'-dpdf');
              printfig(hf,fullfile(FinalExpPath,[StampMeanV StrainLabels{j} '.png']),'-dpng');
        case 'medianV'
...
function SortandPutLeg(P,Handles,Labels)
         ...
         leg = legend([Handles{LegendOrder}],Labels{LegendOrder},'Location','NorthEast')
         leg.FontSize = 10; 
end